...
public class TP5_2Tests extends TestCase {
	...
	public void testConsulterEmploye() throws Exception {
		...
		for ( Poste p : e.getPostes() ) {
			System.out.println("Poste :" + p.getLibelle() + " mission :" + p.getMission().getLibelle());
		}
		tx.commit();
		em.close();
	}
	...
	public void testCreerUnPoste() throws Exception {
		...
		Employe e = em.find(Employe.class,new Long(2));
		...
		Mission m = em.find(Mission.class,new Long(3));
		...
		Poste p = new Poste(e,m);
		p.setCharge(4.5f);
		p.setLibelle("Développeur");
		em.persist(p);
		tx.commit();
		em.close();
	}
